...
def is_in_quest(emulator_device):
    ...
    logger.debug(f"{adbutil.full_name(emulator_device)}: is {'' if is_it else 'NOT'} in quest")
    return is_it
...
def is_in_tavern(emulator_device):
    ...
    logger.debug(f"{adbutil.full_name(emulator_device)}: is {'' if is_it else 'NOT'} in tavern")
    return is_it
...
def is_in_quest_selection(emulator_device):
    ...
    logger.debug(f"{adbutil.full_name(emulator_device)}: is {'' if is_it else 'NOT'} in quest select")
    return is_it
...
def is_close_ad_text_present(emulator_device, image_path):
    imgutil.enhance_contrast(image_path, imgutil.get_contrasted_image_path(image_path), enhance_contrast=1)
    ...
    for img_path in image_paths:
        for psm in const.PSM_CONFIG:
            ...
            for char in const.CLOSE_BUTTON_WHITELIST_STRING:
                if char in text:
                    logger.debug(f"{adbutil.full_name(emulator_device)}: close ad found by char: {char}")
                    return True

    logger.debug(f"{adbutil.full_name(emulator_device)}: X in ad not found as a text")
    return False
...
def is_close_ad_present(emulator_device):
    ...
    for image in const.LIST_OF_CLOSEBUTTONS:
        if ocrutil.are_images_similar(emulator_device,
                              ...
                              const.CLOSE_AD_DIFF_THRESHOLD):
            logger.debug(f"{adbutil.full_name(emulator_device)}: close ad found by image")
            return True

    logger.debug(f"{adbutil.full_name(emulator_device)}: close ad {'' if is_it else 'NOT'} found by any way")
    return False
...
def is_selected_correct_quest(emulator_device, quest_num):
    ...
    logger.debug(
        f"{adbutil.full_name(emulator_device)}: current "
...
def is_enough_thirst(emulator_device):
    ...
    logger.debug(f"{adbutil.full_name(emulator_device)}: {'' if is_it else 'NOT'}enough thirst")
    return is_it
...
def is_quest_ad_wo_hourglass_present(emulator_device):
    ...
    logger.debug(f"{adbutil.full_name(emulator_device)}: Quest ad is with{'out' if is_it else ''} hourglass")
    return is_it
...
def is_quest_ad_present(emulator_device):
    ...
    is_it = ocrutil.are_images_similar(emulator_device,
                               ...
                               const.MENU_BUTTON_IMAGE_DIFF_THRESHOLD)
    logger.debug(f"{adbutil.full_name(emulator_device)}: Quest is {'' if is_it else 'NOT'} skippable with free ad")
...
def is_quest_done(emulator_device):
    ...
    logger.debug(f"{adbutil.full_name(emulator_device)}: Quest is {'already' if is_it else 'not yet'} done")
    return is_it
...
def is_new_level_accept_present(emulator_device):
    ...
    logger.debug(f"{adbutil.full_name(emulator_device)}: New level button is {'' if is_it else 'not'} present")
    return is_it
...
def is_in_profile_selection(emulator_device):
    ...
    logger.debug(f"{adbutil.full_name(emulator_device)}: Is {'' if is_it else 'not'} in profile select")
    return is_it
...
def can_drink_more(emulator_device, can_use_mushrooms=const.CAN_USE_MUSHROOMS_FOR_BEER):
    if can_use_mushrooms:
        ...
        logger.debug(f"{adbutil.full_name(emulator_device)}: Not drinking beer, the mushroom button is not present.")
    else:
        logger.debug(f"{adbutil.full_name(emulator_device)}: Not drinking beer, if you want, set CAN_USE_MUSHROOMS to True")
